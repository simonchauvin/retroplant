        ...
        if (ready)
        {
            ...
            if ((index > 0 && nodes[index].depth + 1 <= startingAge) || index <= 0)
            {
                ...
                if (currentNode.getChildren().Count > 0)
                {
                    ...
                    while (Mathf.Abs(currentNode.getChildren()[0].angle - currentAngle) < currentNode.maxAngle * wideness)
                    {
                ...
                newNode.init(unitSize.y, currentAngle, currentNode.maxAngle * angleDecreaseFactor, length, currentNode, depth);
